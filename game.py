...
class Game:
    def __init__(self, team1, team2, pitchers1, pitchers2):
        ...
        for te in data:
            ...
            print(bat.Batter.average_batter)

        for stat in bat.Batter.average_batter:
            bat.Batter.average_batter[stat] /= float(bat.Batter.league_pa)
            print(bat.Batter.average_batter[stat])

        #get list of active players
        response = requests.get("https://statsapi.mlb.com/api/v1/sports/1/players?season=2019")
        players = response.json()

        self.home = team.Team(team1, pitchers1, players)
        self.away = team.Team(team2, pitchers2, players)
...
for x in f1:
    if pos < 9:
        team1.append(x.strip())
        pos += 1
    else:
        pitchers1.append(x.strip())

pos = 0
for x in f2:
    if pos < 9:
        team2.append(x.strip())
        pos += 1
    else:
        pitchers2.append(x.strip())

f1.close()
f2.close()
print(pitchers2)
g = Game(team1, team2, pitchers1, pitchers2)
g.play()
